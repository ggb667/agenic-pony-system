...
- Step 2 progress: Agenic commits `7834278`, `5760b6d`, and `40a4213` prove managed config export across all personalities and now supply both explicit `globalSingleton` and `mailboxPath`. AJ's reviewed Codex commit `cd5542ec7b` removes the hard-coded Celestia singleton fallback with 17/17 focused tests passing. Remaining work is to consume configured mailbox policy and remove remaining Pony-specific errors, names, and default layout from generic `agent_ipc` without breaking wire compatibility.
