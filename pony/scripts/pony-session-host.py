...
def startup_brief_prompt() -> str:
    return (
        "Startup behavior: on your first turn, greet the developer in character with a concise startup self-brief. "
        "Cover your pony identity, stable role, active project and workspace, startup phase ORIENTATION_REQUIRED, "
        ...
        "After that first-turn self-brief, perform read-only orientation "
        ...
        "Orientation only establishes context; do not begin implementation, alter files, send coordination messages, repair a preflight, deploy, or rerun anything. "
        "Historical state never authorizes work merely because it was read. If orientation sources conflict, report the exact sources and conflicting values to Twilight and remain parked. "
        "Begin work only after an explicit post-start instruction from the user or Twilight. "
...
class PonySessionHost:
    ...
    def _resolve_preflight(self) -> tuple[str, list[str], str]:
        ...
        return result, codex_args, startup_brief_prompt()
