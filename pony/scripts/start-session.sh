...

{
  ...
  printf '%s\n' "Memory rule: keep the first-turn startup self-brief cheap, but once real work, routing, or follow-up begins, read the Assigned memory capsule first when it exists, then reconcile it against the Assigned workfile and authoritative local coordination state before acting."
  printf '%s\n' "Current-condition rule: if the startup handoff includes any concrete Current condition task, routing issue, or follow-up action, do not treat the self-brief as a stopping point; immediately begin that post-brief initialization before answering or asking permission."
  ...
  if [[ "$personality" != "TWILIGHT_SPARKLE" && "$personality" != "PRINCESS_CELESTIA_SOL_INVICTUS" ]]; then
    ...
    printf '%s\n' "Memory-capsule startup rule: after the no-tool self-brief, read the Assigned memory capsule first when it exists, then the Assigned workfile, then the shared status and any relevant coordinator state needed for the current task, and do so before answering or asking permission when a concrete Current condition is already assigned."
