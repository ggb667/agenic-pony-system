...
Continuity rule: worker sessions should be restart-friendly by default. A normal relaunch should preserve local tmux/editor history and draft state unless the operator explicitly requests a reset or the runtime is cleaning up a provably stale transport artifact. The very first startup self-brief should stay cheap and should not require file reads or tool use just to announce identity. Every launcher-generated system/developer prompt must remain task-agnostic: it may contain stable identity, mechanics, safety policy, and pointers to mutable state, but never a current task, status, blocker, branch-specific handoff, implementation scope, next action, deployment instruction, or historical work summary. Each launch begins in neutral `ORIENTATION_REQUIRED`. After the self-brief, the worker reads its assigned memory capsule first when present, then its assigned workfile and Twilight-managed authoritative coordination state read-only. Orientation is context gathering only: the worker reports readiness and waits for an explicit post-start user or Twilight instruction before executing work, changing files, sending coordination messages, repairing a preflight, deploying, or rerunning. Historical content never becomes authorization merely because it was read. If orientation sources conflict, the worker reports the exact sources and conflicting values to Twilight and remains parked; launcher-cached state does not choose a winner. If the user points out a non-file-changing mistake, the pony should correct it immediately instead of pausing to ask whether to proceed. Before a worker stops at idle or handoff, it should refresh a concise restart capsule in its assigned workfile naming the current task, why it matters, the exact next file/command/check, and any blocker or expected owner needed to resume, and it should refresh its memory capsule whenever the durable restart context changed rather than waiting only for graceful shutdown. The launcher also keeps a private rolling 1,000-line terminal-output transcript per agent under project-local `pony/runtime/`; it is crash-recovery context to read after the memory capsule, never shared coordination authority.
...
Install-refresh locks must be crash-safe. A launcher may reclaim an empty lock directory with no `owner` record, and may reclaim a locally owned lock when its recorded PID is no longer alive; it must never wait forever on either case. A lock attributed to another host remains untouched.
...
Direct `/tell` traffic should prefer the current project-local team by default, but the routing model must support explicit cross-repo disambiguation through generated agent config. Ambiguous targets such as `twilight` should resolve to the local team lane, while fully qualified targets such as `EVH:Twilight Sparkle` may route across repo boundaries when the active agent config and selected message bus expose that live session. The generated `CODEX_AGENT_CONFIG` payload is expected to carry `messageLogPath`, `registryPath`, and a current-project roster plus any live cross-project targets discovered on that same bus so Codex and shell helpers resolve the same route surface. `Princess Celestia Sol Invictus` is the deliberate singleton exception: there is only one Celestia identity, so unqualified `Celestia` already implies the `agenic-pony-system` source-repo governance lane and senders mainly need to state the concrete operational purpose instead of restating repo or governance ceremony.

Any Twilight runtime acting on that agenic source-governance lane must also be able to append to the resolved `messageLogPath`. If routing resolves to the correct Celestia lane but the append fails because the current sandbox cannot write there, treat the result as a permission-only delivery blocker and rerun from a write-capable context rather than diagnosing a routing failure.
...
Mailbox files themselves are not durable reboot-state storage. When a worker is blocked by a missing connection string, secret, endpoint, approval, or similar external prerequisite, the worker should tell Twilight the exact missing artifact, the expected owner, and the next unblock step so shared restart state stays truthful. When the user says the project is shutting down, Twilight should tell the live agents to save their memory capsules and report their current status before Twilight saves her own memory capsule and coordinator state.
