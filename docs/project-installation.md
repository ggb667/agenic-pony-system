...
Continuity rule: worker sessions should be restart-friendly by default. A normal relaunch should preserve local tmux/editor history and draft state unless the operator explicitly requests a reset or the runtime is cleaning up a provably stale transport artifact. The very first startup self-brief should stay cheap and should not require file reads or tool use just to announce identity. Once that self-brief is complete and startup context names a task, routing question, or follow-up, the launcher and prompt stack should direct the worker to perform read-only orientation: read its assigned memory capsule first when present, then its assigned workfile and authoritative local coordination state. A `Current condition` must not make the self-brief a stopping point, but it is context rather than standing authorization: after orientation, the worker reports readiness and waits for an explicit user or Twilight instruction before executing work, changing files, sending coordination messages, or remedying a preflight. If the memory capsule is blank, stale, malformed, or contradictory, the worker should continue from the richer authoritative state and refresh the capsule instead of blocking. If the user points out a non-file-changing mistake, the pony should correct it immediately instead of pausing to ask whether to proceed. Before a worker stops at idle or handoff, it should refresh a concise restart capsule in its assigned workfile naming the current task, why it matters, the exact next file/command/check, and any blocker or expected owner needed to resume, and it should refresh its memory capsule whenever the durable restart context changed rather than waiting only for graceful shutdown. The launcher also keeps a private rolling 1,000-line terminal-output transcript per agent under project-local `pony/runtime/`; it is crash-recovery context to read after the memory capsule, never shared coordination authority.
