...
- once that self-brief is complete and startup context names a task, routing question, or follow-up, the worker should perform read-only post-brief orientation by reading its assigned memory capsule first when present, then its assigned workfile, shared status, and relevant coordinator state; it should then report readiness and wait for an explicit user or Twilight instruction before acting
- if the startup handoff includes a concrete `Current condition`, the worker should not treat the self-brief as a stopping point and should complete read-only orientation before answering; that context is not standing authorization to execute work, modify files, send messages, or remedy a preflight
