...
- every launch should declare only the neutral phase `ORIENTATION_REQUIRED`; after the self-brief, the worker should perform read-only orientation by reading its assigned memory capsule first when present, then its assigned workfile, shared status, and relevant Twilight-managed coordinator state
- launcher-generated system/developer prompts must contain stable identity, mechanics, safety policy, and pointers only; they must never contain a current task, status, blocker, branch-specific handoff, implementation scope, next action, deployment instruction, or historical work summary
- orientation is context gathering, not authorization: the worker must not execute work, modify files, send work messages, repair preflight, deploy, or rerun until an explicit post-start user or Twilight instruction arrives
- if orientation sources conflict, the worker should report the exact sources and conflicting values to Twilight and remain parked; launcher-cached state must not choose a winner
- if the user points out a non-file-changing mistake, the pony should correct it immediately instead of asking whether to proceed
- before explicit task execution, an agent on a clean `main` worktree must run `git fetch --prune origin` and `git pull --ff-only`; it must not pull a dirty worktree, a non-`main` branch, or non-fast-forward history, and should record the exact state for Twilight instead
- a pre-existing linked worker worktree is not a fresh assignment starting point: Twilight must confirm project `main` was refreshed before explicitly preparing the worker branch/worktree, without resetting, rebasing, or discarding dirty worker state
...
- if another pony must act: send that pony a direct `/tell`
- if shared durable state must change: tell Twilight the exact update to record
- if both are true: do both in the same run

Mailbox files are notification lanes, not restart-state storage. If a fact must survive reboot, it must be recorded by Twilight in the shared authoritative coordination mechanism from an exact worker write request.

When a worker is blocked on a missing connection string, secret, endpoint, approval, or similar external prerequisite, the shared authoritative state Twilight maintains should record:
